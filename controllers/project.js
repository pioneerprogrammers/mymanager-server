...
exports.newProject = async (req, res) => {
  const { projectName, projectThumnail, startDate, dateline, budget } =
    req.body;
  const { user } = req;
...
exports.updateProject = async (req, res) => {
  ...
  try {
    ...
    project.projectThumnail = projectThumnail
      ? projectThumnail
      : project.projectThumnail;
...
exports.getProjects = async (req, res) => {
  try {
    ...
    const data = {
      ...
      total:
        projects[0].metadata.length > 0 ? projects[0].metadata[0].total : 0,
      noOfPage:
        projects[0].metadata.length > 0 ? projects[0].metadata[0].page : 0,
    };
