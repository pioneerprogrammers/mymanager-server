...
exports.newTask = asyncHandler(async (req, res) => {
  const { taskName, startDate, points, repeat, allowAsNa, isActive, _id } =
    req.body;
  try {
  ...
  try {
    ...
    if (tasks.length > 0) {
      ...
      if (hasNotSchedule) {
        ...
        // insert many
        const newScheduleTasks = await ScheduleTask.insertMany(
          taskScheduleList
        );
...
exports.saveTaskCheckListTodos = async (req, res) => {
  ...
  try {
    ...
    // find changed
    const updateableCheckListAns = checkList.filter(
      (todo) => todo._id !== "" && todo.touched
    );
